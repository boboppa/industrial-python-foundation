...
def create_ml_feature_table(events_with_severity_flags):
    ml_feature_table = events_with_severity_flags[["area","equipment","criticality","temperature","temperature_threshold","rolling_mean_temperature","temperature_deviation","is_warning","is_critical"]].copy()
    return ml_feature_table

def encode_categorical_features(ml_feature_table):
    encoded_feature_table = pd.get_dummies(ml_feature_table,columns=["area","equipment","criticality"])
    return encoded_feature_table

def convert_boolean_features(encoded_feature_table):
    boolean_feature_table = encoded_feature_table.copy()
    boolean_feature_table[["area_PM1","area_PM2","area_PM3","equipment_Compressor_A","equipment_Motor_A","equipment_Pump_A","equipment_Pump_B", "criticality_high", "criticality_low","criticality_medium","is_warning","is_critical"]] = boolean_feature_table[["area_PM1","area_PM2","area_PM3","equipment_Compressor_A","equipment_Motor_A","equipment_Pump_A","equipment_Pump_B", "criticality_high", "criticality_low","criticality_medium","is_warning","is_critical"]].astype(int)
    return boolean_feature_table

def add_target_column(boolean_feature_table):
    target_column = boolean_feature_table.assign(
        target_critical = boolean_feature_table["is_critical"] 
    )
...
def main():
    ...
    ml_feature_table = create_ml_feature_table(events_with_severity_flags)
    encoded_feature_table = encode_categorical_features(ml_feature_table)
    boolean_feature_table = convert_boolean_features(encoded_feature_table)
    target_column = add_target_column(boolean_feature_table)
    print(target_column)
    #boolean_feature_table.info()
